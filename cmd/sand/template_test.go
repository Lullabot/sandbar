...
func TestHeadlessRecreateRecognizesTemplateBaseWithoutCachedProvenance(t *testing.T) {
	reg := registry.NewEmpty()
	cfg := vm.CreateConfig{
		Name:     "dev",
		BaseName: vm.TemplateInstanceName("golden"),
	...
	// A second controller can reconstruct ownership from the provider marker
	// while lacking the original registry-only TemplateSource field. The reserved
	// base name must still route the recreate through the template clone path.
	if err := reg.AddScoped(cfg, registry.LocalScope); err != nil {
		t.Fatalf("seed registry: %v", err)
	}
	prov := &stubProvisioner{}
	if err := doHeadlessCreate(context.Background(), reg, prov, cfg, registry.LocalScope, true, false, "", io.Discard); err != nil {
		t.Fatalf("doHeadlessCreate recreate: %v", err)
	}
	if prov.opts.TemplateSource != cfg.BaseName {
		t.Fatalf("TemplateSource = %q, want %q", prov.opts.TemplateSource, cfg.BaseName)
	}
	if got, ok := reg.TemplateSourceInScope(cfg.Name, registry.LocalScope); !ok || got != "golden" {
		t.Fatalf("recorded template provenance = %q, %v; want golden, true", got, ok)
	}
}

func TestHeadlessRecreateRejectsTemplateOverride(t *testing.T) {
	prov := &stubProvisioner{}
	err := doHeadlessCreate(context.Background(), registry.NewEmpty(), prov, vm.CreateConfig{Name: "dev"}, registry.LocalScope, true, false, "golden", io.Discard)
	if err == nil || !strings.Contains(err.Error(), "cannot be combined") {
		t.Fatalf("doHeadlessCreate error = %v, want incompatible flags error", err)
	}
	if prov.called {
		t.Fatal("provisioner was called for an unsupported --recreate --template combination")
	}
}
