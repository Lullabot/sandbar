...
- **Claude Code sessions see the scoped secrets of the directory they work
  in.** The provisioned settings carry `SessionStart`/`CwdChanged` hooks
  that load direnv's exports, so a session in a repo gets that repo's
  [scoped `.env` secrets](../using-sand/secrets.md) just like an interactive
  shell would. Scope your tokens accordingly — a secret placed at a scope is
  available to any agent working under it.
