...
| `${XDG_DATA_HOME:-~/.local/share}/sandbar/managed-vms.json` | The secret-free index of every `sand`-managed VM and golden template: which VMs `sand` created, the base image each was cloned from, each VM's create configuration (the settings that pre-fill the form when you `Reset`), and the list of saved golden templates with their provenance and metadata. Since schema version 3, each entry is keyed by **(connection scope, name)** rather than by bare name, so the same VM name can exist under two different connection profiles without colliding. Schema version 4 (additive on v3) adds a `templates` array and per-VM `templateSource` field recording which template (if any) each VM was cloned from. A v3 file has no templates and loads as an empty template set; it is auto-upgraded to v4 on write. | Yes, but you lose the pre-filled reset config, `sand`'s record of which VMs it manages, and any saved templates — a VM's actual disk is untouched, and Lima will still list it, but `sand` will no longer treat it as managed. |
...
Sources: `internal/profiles/store.go` (`profiles.yaml` default path, schema, and CRUD), `internal/registry/registry.go:53` and `internal/registry/registry.go:62` (`managed-vms.json` default path), `internal/registry/registry.go:109` (schema v4 with templates array), `internal/registry/registry.go:172` (the config saved into that index is the create config, stripped of the clone token), `internal/registry/registry.go:107`-`internal/registry/registry.go:129` (the v3 `(scope, name)`-keyed array shape), `internal/secrets/secrets.go:124` and `internal/secrets/secrets.go:133` (`secrets.json` default path), `internal/secrets/secrets.go:342`-`internal/secrets/secrets.go:350` (directory forced to `0700`) and `internal/secrets/secrets.go:357`-`internal/secrets/secrets.go:359` (file created at `0600`), `internal/provision/baseversion.go:62`-`internal/provision/baseversion.go:68` (`LIMA_HOME` fallback to `~/.lima`), `internal/vm/template.go` (template instance name prefix and reserved naming).

### Golden template instances

Golden templates are stored as reserved Lima instances under `${LIMA_HOME}`, with names following the pattern `sandbar-tmpl-<name>` (e.g., `sandbar-tmpl-golden`, `sandbar-tmpl-next-gen`). The prefix `sandbar-tmpl-` is reserved and never used for ordinary managed VMs, so a template's instance name can never collide with a user VM.

| Path | What it holds | Safe to delete? |
| --- | --- | --- |
| `${LIMA_HOME}/instances/sandbar-tmpl-<name>` | The Lima instance storing the template's guest disk. This is a read-only clone source — not a working VM. Deleting it directly breaks any VMs cloned from this template (they keep working, but can no longer be recreated). Use `sand template delete <name>` instead. | Not directly — use `sand template delete <name>`. |
| `${LIMA_HOME}/_sand/<name>.playbook-version` | Metadata: the playbook version stamp recorded when this template was snapshotted (see `internal/provision/baseversion.go`), used by `sand template list` to report the template's status (`current`, `stale`, `unknown`). | Yes, but `sand template list` will then report the template's status as `unknown`. |

Sources: `internal/vm/template.go` (template instance name prefix), `internal/registry/registry.go:142`-`internal/registry/registry.go:154` (diskTemplate schema with Name, Scope, Source, CreatedAt, PlaybookVersion, ToolsetKey, Config).
