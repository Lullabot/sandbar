...
| `${XDG_DATA_HOME:-~/.local/share}/sandbar/managed-vms.json.lock`, `…/secrets.json.lock` | Empty advisory lock files, one per state file above. Every write takes its file's lock, re-reads the file, applies the change and writes it back, so two `sand` processes (a TUI you left open and a `sand create` in another terminal) cannot overwrite each other's entries. They hold no data. | Yes — they are re-created on the next write. Deleting one while `sand` is running just means that write is unserialized. |
