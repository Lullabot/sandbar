...
Golden templates are named, reusable clone sources captured from an existing sand-managed VM. Snapshot an existing VM once you've provisioned it to your liking, then create new VMs from that template instead of rebuilding the shared base image. Sand still runs its finalize pass so the clone receives its selected identity and settings.
...
The snapshot runs in the background (just like a create or reset). Its progress streams into the progress pane; you can leave and reopen its retained log.
...
The `--template` flag is mutually exclusive with `--rebuild` (templates are already cloned, not rebuilt from base), `--base-name` (the template instance is the clone source), and `--recreate` (a recreate always uses the VM's recorded source).

### TUI

1. Press `n` (or `enter` on the ghost tile) to start a create.
2. A "Source" field appears in the form, defaulting to the shared base image. Use left/right to cycle through the base and templates in that profile.
3. Leave the base selected or choose a template name.
...
1. Open the create form and focus its Source row.
2. Select the template and press `d`.
3. If there are dependents, a warning lists them. Confirm the delete.

## Secrets and identity propagation

### What propagates

Everything installed or configured in the guest disk propagates into clones.
The normal finalize pass reapplies the clone's selected hostname and git
identity, and the provider establishes access for the new VM, but those are
post-clone updates rather than exclusions from the snapshot.
