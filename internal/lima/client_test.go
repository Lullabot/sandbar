...
func TestMethodArgv(t *testing.T) {
	cases := []struct {
		...
		{"configure", func(c *Client) { _ = c.Configure("vm1", 4, "8GiB", "100GiB", "/host/playbook") },
			[]string{"edit", "--set", `.cpus=4 | .memory="8GiB" | .disk="100GiB" | .mounts |= map(select(.writable != true)) | (.mounts[] | select(.mountPoint == "/mnt/playbook") | .location) = "/host/playbook"`, "vm1"}},
