...
func TestCreateVMWithOptions_TemplateSource_ClonesFromTemplateSkipsBase(t *testing.T) {
	...
	want := [][]string{
		...
		{"edit", "--set", `.cpus=4 | .memory="8GiB" | .disk="100GiB" | .mounts |= map(select(.writable != true)) | (.mounts[] | select(.mountPoint == "/mnt/playbook") | .location) = "/playbook"`, "claude"}, // Configure clone sizes and point the playbook mount at this checkout.
		{"start", "claude"},
