...
// templateRestoreTimeout bounds the cancellation-detached restart that restores
// a running snapshot source. Restoring the user's VM is cleanup, so it must
// survive cancellation of the snapshot itself, but it must not block forever.
const templateRestoreTimeout = 15 * time.Minute
...
func (p *Provisioner) SnapshotTemplate(ctx context.Context, source, templateInstance string, out io.Writer) (SnapshotResult, error) {
	...
	prior, err := p.Lima.Status(source)
	if err != nil {
		return SnapshotResult{}, fmt.Errorf("read %q power state before snapshotting it: %w", source, err)
	}
	wasRunning := prior == "Running"

	// Register restoration before the stop begins. A canceled stop can still
	// have powered the guest off before returning its context error, and cleanup
	// must cover that ambiguous result as well as every later failure.
	defer func() {
		if !wasRunning {
			return
		}
		restoreCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), templateRestoreTimeout)
		defer cancel()
		if err := p.Lima.StartStreaming(restoreCtx, source, out); err != nil {
