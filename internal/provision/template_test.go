...
func TestSnapshotTemplate_CancelledStopStillRestartsSource(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	f := &fakeRunner{status: map[string][]byte{"claude": []byte("Running\n")}}
	f.hook = func(args []string) {
		if callIndex([][]string{args}, "stop", "claude") == 0 {
			cancel()
		}
	}
	p := &Provisioner{Lima: lima.New(f), PlaybookDir: "/playbook"}

	_, err := p.SnapshotTemplate(ctx, "claude", "sandbar-tmpl-golden", io.Discard)
	if !errors.Is(err, context.Canceled) {
		t.Fatalf("SnapshotTemplate error = %v, want context cancellation", err)
	}
	if callIndex(f.snapshot(), "start", "claude") == -1 {
		t.Fatalf("source was not restarted after cancellation; calls=%v", f.snapshot())
	}
}

func TestSnapshotTemplate_StatusFailureDoesNotMutateSource(t *testing.T) {
	f := &fakeRunner{
		status: map[string][]byte{"claude": []byte("Running\n")},
		failOn: func(args []string) bool {
			return len(args) >= 2 && args[0] == "list" && args[1] == "claude"
		},
	}
	p := &Provisioner{Lima: lima.New(f), PlaybookDir: "/playbook"}

	if _, err := p.SnapshotTemplate(context.Background(), "claude", "sandbar-tmpl-golden", io.Discard); err == nil {
		t.Fatal("SnapshotTemplate succeeded despite a source status failure")
	}
	calls := f.snapshot()
