...
func TestE2E_ConfigureGrowsDiskAndStageRoundTrip(t *testing.T) {
	...
	if err := StageOut(context.Background(), cli, clone, home, user, []string{".claude", ".claude.json"}, archive, "Claude data", io.Discard); err != nil {
	...
	if err := StageIn(context.Background(), cli, clone, home, user, []string{".claude", ".claude.json"}, archive, "Claude data", io.Discard); err != nil {
