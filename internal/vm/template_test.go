...
func TestTemplateNameFromInstance(t *testing.T) {
	name, ok := TemplateNameFromInstance("sandbar-tmpl-web-v2")
	if !ok || name != "web-v2" {
		t.Fatalf("TemplateNameFromInstance = %q, %v; want web-v2, true", name, ok)
	}
	for _, invalid := range []string{"sandbar-base", "sandbar-tmpl-", "sandbar-tmpl-Bad"} {
		if IsTemplateInstanceName(invalid) {
			t.Errorf("IsTemplateInstanceName(%q) = true, want false", invalid)
		}
	}
}
