...
// TemplateNameFromInstance reverses TemplateInstanceName for a valid reserved
// template instance name. The returned name is already normalized because the
// on-disk/backend name stores the slug, not the user's original spelling.
func TemplateNameFromInstance(instanceName string) (string, bool) {
	name, ok := strings.CutPrefix(instanceName, templateInstancePrefix)
	if !ok || name == "" || !templateNameRE.MatchString(name) {
		return "", false
	}
	return name, true
}

// IsTemplateInstanceName reports whether name belongs to the namespace reserved
// by TemplateInstanceName.
func IsTemplateInstanceName(name string) bool {
	_, ok := TemplateNameFromInstance(name)
	return ok
}
