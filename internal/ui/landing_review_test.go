...
func TestLandingQuitCmdCancelsInFlightReview(t *testing.T) {
	...
	if _, ok := msg.(tea.QuitMsg); !ok {
		t.Fatalf("quit() = %T, want tea.QuitMsg", msg)
	}
	select {
	case <-ctx.Done():
	case <-time.After(2 * time.Second):
		t.Fatal("quit did not cancel the in-flight review before quitting")
	}
}
...
// quit() must not return the QuitMsg until that goroutine has actually
...
func TestLandingQuitCmdWaitsForTeardownBeforeQuitting(t *testing.T) {
	...
	if elapsed < teardown {
		t.Fatalf("quit() returned after %s, want it to have waited out the %s teardown", elapsed, teardown)
	}
}

// TestLandingQuitCmdGivesUpAfterTeardownTimeout proves the quit path does not hang
...
func TestLandingQuitCmdGivesUpAfterTeardownTimeout(t *testing.T) {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("quit() did not give up after quitTeardownTimeout — a wedged guest would hang the program forever")
	}
}

// TestLandingQuitCmdWithNoReviewInFlightStillQuits is the quit path's zero-value case: no
...
func TestOpenLandingPaneKeepsAnInFlightReviewsTeardownHandles(t *testing.T) {
	...
	if m.review.done == nil {
		t.Fatal("the review's done channel was dropped by reopening the pane — the quit path could no longer wait for teardown")
	}

	// And the handles must still be the LIVE ones: the quit path has to reach the
	// real session, not a zero value that merely looks non-nil.
	close(done)
	if msg := m.quit()(); msg == nil {
		t.Fatal("quit produced no message")
	}
	select {
	case <-cancelled:
	default:
		t.Fatal("quit did not cancel the review that survived the pane reopen")
	}
}
