...
type model struct {
	...
	// formTemplateName is the selected clone source. Empty means the shared
	// base image; a value names a golden template in formScope.
	formTemplateName string
	// formSourceRows caches template metadata for the selected profile.
...
func (m model) dispatch(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case provisionDoneMsg:
		...
		// A non-canceled failure is otherwise surfaced only on the full-screen
		// progress/log view and the tile's Failed badge — someone watching the
		// board (not that screen) never sees WHY it failed. Echo it into the
		...
		if msg.err == nil && msg.job.kind == kindProvision && isProvision && cfg.Name != "" {
			...
			// Reversing cfg.BaseName (the backend-reserved template instance name)
			// recovers the normalized user-facing name without requiring the
			// template's registry entry to survive for the duration of the build.
			//
			...
			tname, _ := vm.TemplateNameFromInstance(cfg.BaseName)
