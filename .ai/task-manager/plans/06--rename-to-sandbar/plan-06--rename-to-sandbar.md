---
id: 6
summary: "Rename the project from deviantintegral/claude-code-ansible to lullabot/sandbar (binary `sand`): update the Go module path and its 36 import sites, all repo/install/CI URLs, the app/binary name, every internal claude-vm-* artifact (incl. the coupled /dev/shm vars file and /var/log provision logs), and the XDG data dir, with a migrate-then-cleanup of the managed-VM index. The GitHub org transfer is a human-performed prerequisite; identity is pre-verified."
created: 2026-06-29
---
...
| Go module `github.com/deviantintegral/claude-code-ansible/tui` (36 import sites across 19 files) | `github.com/lullabot/sandbar/tui` | Module path must match the repo location to be `go get`-able and self-consistent |
...
| Internal artifacts named `claude-vm-*`: temp prefixes (`claude-vm-reset-*`, `claude-vm-base-*`, `claude-vm.XXXXXX`), `/dev/shm/claude-vm-vars.yml`, `/var/log/claude-vm-{provision,finalize}.log`, and ~20 code comments | All renamed to `sand-*` (e.g. `/dev/shm/sand-vars.yml`, `/var/log/sand-{provision,finalize}.log`), with coupled host↔guest↔CI pairs moved together | "Full sweep" scope — the repo-wide grep gate expects zero `claude-vm`; coupled pairs break if only one side moves |
...
- **The module path is load-bearing in 36 import lines** across 19 files in `internal/lima`, `internal/provision`, `internal/registry`, `internal/ui`, and `cmd/claude-vm`. A single `go.mod` edit plus an import rewrite covers them; the build + `go test ./...` prove completeness.
...
flowchart TD
    ...
    A["1. Go module + binary rename<br/>go.mod path, 36 imports, cmd/claude-vm -> cmd/sand, TUI title/help"]
...
- **Internal-artifact naming convention:** `claude-vm-*` → `sand-*` (`/dev/shm/sand-vars.yml`, `/var/log/sand-{provision,finalize}.log`, `sand-reset-*`, `sand-base-*`, `sand.XXXXXX`).
