...
## Re-validate task models/effort (refinement only)

If tasks already exist for this plan (this is a refinement, not a fresh plan),
re-check each task's `models` and `effort` frontmatter against
...
trimmed. Migrate an active task's legacy scalar `model` field to the provider
`models` map when touching its tier; leave unchanged tasks alone. When no tasks
exist yet, skip this step; tiers are assigned at task generation by
