...
| When does the repo sweep run? | Always, on the heartbeat loop at slow cadence — so the badge and delete guard are accurate for running VMs with no user action. |
...
checkout registry**: the existing guest heartbeat loop — already the one honest
source of guest-side truth — gains a slow-cadence repository sweep that discovers
every git checkout (and worktree) under the guest home, records each one's branch
and unpushed/uncommitted state on the host, and keeps it fresh while the VM runs.
...
bundle or patch, view a diff — each firing into the guest over the existing
provisioning/transfer machinery and each recorded in the job registry as a
reopenable ledger entry.
...
| The heartbeat loop samples `/proc` stats and disk only. | The same loop also sweeps for checkouts at a slower cadence and streams their state. | Reuse the one existing guest telemetry channel rather than adding a guest daemon. |
...
- **Heartbeat** (`internal/ui/heartbeat.go`): one long-lived `limactl shell` per
  running VM runs a guest loop that cats `/proc/stat`, `/proc/meminfo`, and
  `df -kP /`, printing a delimiter each interval; the host parses the stream.
  Its own doc comment prizes a *deliberately dumb* guest side ("a clever guest
  script is a thing that breaks on a distro nobody tested"). The repo sweep must
  honor that: a plain `find` + a handful of `git` porcelain reads, no bespoke
  guest program. Samples reach the model as messages recorded under a mutex in a
  pointer-held registry (Bubble Tea passes the model by value) — the checkout
  data must follow the identical concurrency contract.
...
graph TD
    subgraph Guest [Guest VM]
        HB[Heartbeat loop] -->|every Nth tick| SW[Repo sweep: find .git + git porcelain]
    end
    ...
    VERBS --> BUNDLE[export bundle/patch → download path]
...
The heartbeat's guest loop gains a second, slower cadence (every Nth tick, target
~60s) that runs a bounded discovery: `find` from the guest home for `.git`
...
fixed set of `git --no-optional-locks` porcelain values — current branch,
upstream, ahead/behind counts (`rev-list --count`), and dirty-file count
(`status --porcelain`) — each wrapped in a per-repo `timeout` so one pathological
repo cannot stall the stream. The guest side stays dumb: plain `find` and `git`,
no bespoke script. When the caps truncate the result, that fact is emitted and
surfaced (logged, and reflected in the pane) rather than silently dropping
checkouts.

The host parses this as a distinct delimited section of the heartbeat stream and
records it into a **per-VM checkout registry** that lives beside the existing
sample state: pointer-held, mutex-guarded, updated only from the parser
goroutine via a message applied in `Update` (the same concurrency contract the
heartbeat established). Each row carries: path, kind (repo | worktree), parent
...
connection-scoping so a same-named VM on two profiles never collides.
...
a dirty marker), aggregated across the VM's checkouts. It follows the
heartbeat/gauge philosophy — it shows only what the registry actually observed,
and a VM whose registry is empty or stale shows nothing (or a stale indicator),
never a fabricated state. The badge must fit the existing tile layout and status
bands without disrupting them, and degrade cleanly on a VM that has never been
swept.
...
- **View diff** — always available when commits/changes exist; runs the diff in
  the guest and streams it into the viewport like a build log.
- **Push branch** — offered when the branch is ahead or has no upstream
  (`git push -u origin HEAD`). Independent of opening a PR.
...
- **Push + PR** — the convenience combo, offered when both apply.
- **Export bundle / patch** — always available when history exists; runs
  `git bundle create` (default; preserves history and binaries) or
  `format-patch` in the guest, then lands the artifact on the host via the
  existing download/transfer path.

All guest execution goes through `bash -lc` so the login shell sources
`secrets.env` and `gh` sees `GH_TOKEN`; plain `git push` authenticates via the
existing credential helper from the checkout's directory. Every verb runs as a
**job** so its output streams live and is retained as a **ledger** entry,
reopenable with `L`. A **lazy** check at pane-open (a single `gh pr view --json`
per remote-present branch) lets the pane show "PR #N already open" instead of
offering a duplicate; this check is polish, not correctness (`gh pr create`
already errors cleanly on an existing PR). Forge scope is **`gh` only**:
GitLab/drupal.org checkouts still get push, bundle/patch export, and diff; the
one-key MR adapter (`glab`) is explicitly deferred. `sand land NAME` provides
CLI parity for the verbs, matching the `create`/`shell` command pattern, so a
shell-centric user can land without the board.
...
- **Sweep cost on large or numerous guest filesystems**: an unbounded `find` +
  per-repo git reads could stall the heartbeat or spike guest I/O.
    - **Mitigation**: prune known-noise paths, cap depth and total checkouts
      (~50), wrap each repo's git reads in `timeout`, and run the sweep at a
      slower cadence than the stats tick. Surface truncation instead of hiding
      it.
...
2. Pressing `d` on such a VM shows a delete confirmation that names the unlanded
   ...
   last-seen data.
3. Pressing `l` (and `sand land NAME`) opens the Landing pane listing the VM's
   checkouts with per-checkout state; log-reopen now responds to `L`, and the
   `?` keys screen and tile footer reflect the new mapping.
4. From the pane, on a branch with unpushed commits: **push** publishes the
   branch; on a branch already on the remote: **open PR** creates a draft PR via
   `gh`; **export bundle** produces a `.bundle` on the host via the download
   path; **view diff** streams the diff. Each action appears in the job
   registry/ledger and is reopenable with `L`.
5. A GitLab/drupal.org checkout still offers push, bundle/patch export, and diff
   (no MR verb), confirming forge-agnostic verbs work without the gh adapter.
...
3. From the pane, run **push** on the unpushed branch and confirm the branch
   ...
   `.bundle` is on the host (`git bundle verify`); run **view diff** and confirm
   the diff renders. Reopen each via `L` and confirm the ledger retained the run.
...
- git plumbing/porcelain (`rev-list`, `status --porcelain`, `bundle`,
  `format-patch`, `--no-optional-locks`) and `gh` CLI PR creation.
...
- **Deferred follow-ups (out of scope here):** a `glab` MR adapter for
  GitLab/drupal.org; any richer landing history UI beyond the job-registry
  ledger.
