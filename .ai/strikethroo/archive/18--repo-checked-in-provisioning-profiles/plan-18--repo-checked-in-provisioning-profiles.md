...
- **The branch was pushed and PR #68's CI was actually run** (a `GH_TOKEN` became available mid-session), including a manual `workflow_dispatch` that exercises the normally weekly-only `molecule` and `mutation` jobs on every push — `lint`, `unit`, `mutation`, and `remote-lima-e2e` all passed; `lima-e2e` (the finalize stage's authoritative per-PR guard) is confirmed passing on the fixture-repo/malformed-manifest/toolset-reconciliation checks from local verification and re-confirmed in CI.
- **Found (and fixed) a real bug in the new `molecule/repo-profile` scenario (Task 6)**: molecule invokes `ansible-playbook` with the scenario's own directory as CWD, not the repo root, so the scenario's `ANSIBLE_ROLES_PATH` (guessed relative-to-repo-root at authoring time, never run against real Docker/molecule in this sandbox) resolved to nonexistent nested paths. Fixed to `../../roles:../../shipped-profiles/roles:fixture-checkout/.sandbar/roles` and verified locally by reproducing the exact CI error with the old value and confirming resolution with the new one (`ansible-playbook --syntax-check`/`--check` run from `molecule/repo-profile/`).
- **Discovered a separate, pre-existing bug in `molecule/base` and `molecule/samba`**, both completely untouched by this plan (`git diff main..HEAD` on those paths is empty): the same class of failure — "the role 'X' was not found" — because neither scenario's `molecule.yml` sets any `ANSIBLE_ROLES_PATH`/equivalent at all, and `main`'s `ansible.cfg` has no `roles_path` directive either (molecule doesn't see the repo's `ansible.cfg` regardless). No scheduled or `workflow_dispatch` run of this workflow exists in the repo's history prior to this session, so this appears to be a latent bug that has never actually been exercised by CI before now — not a regression introduced by plan 18. Left unfixed as out of scope for this plan; see Necessary follow-ups. The `molecule` job is weekly/dispatch-only and `continue-on-error: true`, so this does not block PR #68.

### Necessary follow-ups

- **File a separate issue/plan for the pre-existing `molecule/base` and `molecule/samba` role-resolution bug** (see Noteworthy Events) — both scenarios fail with "the role 'X' was not found" on `main` itself, independent of plan 18, apparently because this CI job has never actually been run before. Likely fix: add the same kind of scenario-relative `ANSIBLE_ROLES_PATH` this plan added for `molecule/repo-profile`.
- No scope was deferred from the plan; all four architectural components (manifest schema/validator, shipped-profile restructuring, guest-side finalize stage, verification surface) and all listed documentation targets were completed as scoped, and CI was actually run and observed (not just locally simulated) before this plan's execution concluded.
