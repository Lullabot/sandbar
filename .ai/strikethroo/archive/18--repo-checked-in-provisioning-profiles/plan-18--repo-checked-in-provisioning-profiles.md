...
- **The branch was pushed and PR #68's CI was actually run** (a `GH_TOKEN` became available mid-session), including several manual `workflow_dispatch` runs that exercise the normally weekly-only `molecule` and `mutation` jobs on every push. Final state: `lint`, `unit`, `mutation`, `remote-lima-e2e`, `lima-e2e` (the finalize stage's authoritative per-PR guard), and `molecule (repo-profile)` all pass in real CI — only `molecule (base)`/`molecule (samba)` remain red, and that failure is pre-existing and unrelated (see below).
- **Found and fixed three real bugs in the new `molecule/repo-profile` scenario (Task 6), all only observable against real Docker/molecule** (this sandbox has neither, so Task 6 could only be statically reasoned through at authoring time): (1) molecule invokes `ansible-playbook` with the scenario's own directory as CWD, not the repo root, so the scenario's `ANSIBLE_ROLES_PATH` resolved to nonexistent nested paths — fixed to `../../roles:../../shipped-profiles/roles:fixture-checkout/.sandbar/roles`. (2) `roles/repo-profile`'s `include_vars` for the `.sandbar/profile.yml` manifest is a controller-side action (production never surfaces this since finalize runs `--connection=local` inside the guest, making controller and target the same machine; molecule's docker driver splits them) — fixed by also staging the fixture checkout on the controller via `delegate_to: localhost` in `prepare.yml`. (3) The identical controller-vs-target split hit `apply_shipped_tool.yml`'s `include_vars` for each shipped tool's `profile.yml` one step later — fixed the same way for `scripts/`+`shipped-profiles/`. Each fix was confirmed by re-running the actual `workflow_dispatch` CI job until `molecule (repo-profile)` reported `success`, not just reasoned about statically.
- **Discovered a separate, pre-existing bug in `molecule/base` and `molecule/samba`**, both completely untouched by this plan (`git diff main..HEAD` on those paths is empty): the same class of failure — "the role 'X' was not found" — because neither scenario's `molecule.yml` sets any `ANSIBLE_ROLES_PATH`/equivalent at all, and `main`'s `ansible.cfg` has no `roles_path` directive either (molecule doesn't see the repo's `ansible.cfg` regardless). No scheduled or `workflow_dispatch` run of this workflow exists in the repo's history prior to this session, so this appears to be a latent bug that has never actually been exercised by CI before now — not a regression introduced by plan 18, and confirmed still failing identically across three separate re-dispatches while unrelated to any change this plan made. Left unfixed as out of scope for this plan; see Necessary follow-ups. The `molecule` job is weekly/dispatch-only and `continue-on-error: true`, so this does not block PR #68 (the overall workflow run's conclusion is `success` despite these two job failures).

### Necessary follow-ups

- **File a separate issue/plan for the pre-existing `molecule/base` and `molecule/samba` role-resolution bug** (see Noteworthy Events) — both scenarios fail with "the role 'X' was not found" on `main` itself, independent of plan 18, apparently because this CI job has never actually been run before. The fix this plan applied to `molecule/repo-profile` (an explicit, scenario-relative `ANSIBLE_ROLES_PATH` in each `molecule.yml`) is very likely the same fix `base`/`samba` need, though they may also need the controller-vs-target `include_vars` treatment if either scenario is later changed to load variables that way.
- No scope was deferred from the plan; all four architectural components (manifest schema/validator, shipped-profile restructuring, guest-side finalize stage, verification surface) and all listed documentation targets were completed as scoped, and CI was actually run, iterated on, and observed green (not just locally simulated) before this plan's execution concluded.
