...
### Measured results (task 13, self-validation — 2026-07-14)

Executed live against a real Lima + QEMU/KVM host (Linux, no virtiofsd, sshfs installed on the host to clear a pre-existing Lima boot-time requirement unrelated to this plan — see "Environment notes" below). All numbers below are **VERIFIED**: captured from `sand`'s own per-phase timing summary and directly-inspected guest/host state, not estimated. Every test VM created during this validation (`sand-perf-*`) was deleted afterward; `claude-base` and `test` (pre-existing, belonging to other worktrees) were left untouched.

**A blocking regression was found and fixed during validation** (see below) — the numbers marked "cold build" are measured *after* that fix, because the base could not build at all before it.

#### Before/after table

| Metric | Before (documented in plan / Executive Summary) | After (measured) | Attribution |
| --- | --- | --- | --- |
| Ansible bootstrap | `ansible` bundle: 17.7 MB download / 200.7 MB installed | **NOT ACHIEVED as shipped**: both `ansible-core` (8.2 MB) **and** the full `ansible` bundle are installed — `dpkg -l ansible ansible-core` on a fresh base shows both `ii`. `/usr/lib/python3/dist-packages/ansible_collections` is 436 MB, not absent. Root cause: Debian trixie's `ansible-core` package `Recommends: ansible (>= 12.0.0~)`, and the Lima dependency script's `apt-get install -y ansible-core rsync curl gnupg ca-certificates` (`internal/provision/overlay.go`) does not pass `--no-install-recommends`, so apt pulls the fat bundle in anyway. | **Tier 1 acceptance criterion 2 FAILS.** Not fixed by the validator (see Findings) — costs an estimated 60–90s of unpack/trigger time per base build, folded into the "base image creation" phase. |
| Base-phase apt transactions | 6 separate update+install passes (3 in `roles/base`, 4 in `roles/dev-tools`) | **Confirmed exactly one** `apt-get update`-bearing task ("Refresh apt indexes") and one install task ("Install every base-phase package in a single transaction") run (`changed`) in the base phase; both correctly `skip` in finalize. Verified from the profiled task list of a real cold build. | Tier 1, delivered as designed. |
| Cold base build (all 3 tools, empty host apt cache) | No prior baseline was measured on this host (pre-plan code was not separately checked out and timed — see Limitations) | **TOTAL 9m40.23s** — base image creation 3m19.79s, base playbook 4m18.56s, apt cache harvest 11.57s, base stop 4.74s, clone 31.16s, clone start 57.85s, finalize playbook 16.56s | Measured after the validator's bugfix (below); includes the un-fixed ansible-Recommends cost. |
| Inner loop (playbook content edit, same tool-set, base already built) | Full destroy-and-rebuild from raw Debian (old design) | **TOTAL 3m45.38s**, and critically the **base playbook** phase itself (the part that actually re-converges the playbook) dropped from 4m18.56s to **47.99s** — a ~5.4x reduction. No "base image creation" phase appears (no VM rebuild, no Debian re-download); output shows `Re-applying the playbook to base image "sand-perf-base" (Ansible is idempotent: only the changes are applied)…`. | Tier 1, delivered. Caveat: total wall-clock savings (9m40s → 3m45s, ~61%) is smaller than the playbook-phase savings alone because `clone` + `clone start` (~2m8s) is fixed per-create overhead paid regardless of whether the base changed. |
| Idempotence (locale-gen / authorized_keys) | N/A (collection modules `community.general.locale_gen` / `ansible.posix.authorized_key`) | **Confirmed changed=0** on a second consecutive base-phase run for "Ensure the locale is enabled" (`ok`), "Generate the locale" (`skipping`), "Set default locale" (`ok`), "Fetch SSH authorized_keys from a URL" (`skipping`), "Install fetched authorized_keys" (`skipping`). Whole-play changed count was 7 (not 0) due to unrelated, expected churn: a random-password `lookup()` on the user-creation task (pre-existing, unrelated to this plan) and a create/remove temp-file cycle for GPG keys and the base-speed dpkg flag (by design — removed at the end of every base-phase run). | Tier 1, delivered for the specific criterion asked. |
| Tool-set de-select (Java) — residue on an already-built base | N/A (tool-set didn't exist before) | Re-applying with `--with-java=false` against an already-built (all-3-tools) base: `sand` prints `Note: java were de-selected but remain installed on the base image. Ansible cannot uninstall them. Rebuild the base to remove them (sand create --rebuild, ...)`. `java` binary confirmed **still present** (correct/expected — converge cannot uninstall). `tmux` confirmed present. | Tier 1, delivered exactly as designed. |
| Tool-set de-select (Java) — fresh `--rebuild`, warm cache | N/A | `--rebuild --with-java=false`: `command -v java` confirmed **absent** in the resulting clone. Base playbook phase: 3m23.10s vs 4m18.56s for the all-3-tools cold build — a **55s (~21%) reduction** attributable to excluding the JDK. (Total wall-clock for this run, 9m51.36s, was *not* faster than the all-3-tools baseline — see Limitations on host-contention noise.) | Tier 1, delivered; magnitude smaller than total-time framing might suggest because Java is one of three optional packages and other fixed costs dominate total wall-clock. |
| Clones skip the upgrade (fresh base) | Every clone ran `apt upgrade dist` in finalize | **Confirmed**: "Upgrade all apt packages" reports `skipping` in every finalize run observed (4 separate clones), both for a base under 30 days old. | Tier 1, delivered. |
| Base self-refresh (base aged past 30 days) | N/A | Artificially aged the base's on-disk stamp (`~/.lima/_sand/<base>.playbook-version`) 45 days back, then ran `sand create` with an unchanged playbook/tool-set. Observed: `Base image "sand-perf-base" is older than 30 days; refreshing it once now…`; "Upgrade all apt packages" reports `changed` in the **base** phase and `skipping` in the clone's **finalize** phase; the stamp's `BuiltAt` was rewritten to the current time afterward. TOTAL 5m4.71s. | Tier 1, delivered exactly as designed. |
| Bounce (no reboot required) | Every clone bounced (stop+start) unconditionally after finalize | **No "bounce" phase appeared in any of the 5 live timing summaries captured** (cold build, inner-loop, 2 tool-set runs, self-refresh) — consistent with the conditional-bounce design on a host where nothing required a reboot. The genuine reboot-required trigger and `Reset`'s tmux-safety were **not independently re-verified live** (`Reset` is TUI-only; driving it would have required scripting the Bubble Tea UI) — verified only via the existing unit tests (`TestCreateVM_BouncesWhenRebootRequired`, `TestReset_NoBounceWithoutRebootMarker`, `TestReset_WarnsButStillBouncesOverLiveTmuxSession`), which passed under `go test ./...`. | Tier 1, live-verified for the common (no-bounce) path; reboot-triggered path is unit-test-verified only. |
| Security invariant (writable host mount on a work VM) | N/A (Tier 2 originally proposed a writable mount) | **No writable mount exists anywhere** — `RenderBaseOverlay` never renders one (Tier 2 shipped via a `limactl copy` seed/harvest instead, per task 10's own notes). Verified live on two real clones: `mount \| grep -Ei 'virtiofs\|9p\|sshfs'` shows only the read-only `/mnt/playbook` entry (`ro`); the clones' `lima.yaml` `mounts:` block is byte-identical to the base's and contains only that one read-only entry. The clone-side strip in `Client.Configure` was **mutation-tested**: the `\| .mounts \|= map(select(.writable != true))` clause was deleted, both `TestConfigureArgvStripsWritableMounts` and `TestConfigureStripsWritableMountAgainstRealLimactl` went **red** with the expected failure messages, then the clause was restored and both went **green** again. Diff confirmed clean afterward (`git diff` empty). | Security control confirmed live AND shown able to fail (not assert-only-green). |
| Tier 2 apt cache (re-download elimination on `--rebuild`) | Every base rebuild re-fetched all `.deb` files over the network | **Confirmed eliminated**: with a warm host cache (570 MB, 323 files under `~/.cache/sand/apt-cache/archives`), a full `--rebuild` produced **zero** `Get:` lines in the guest's `/var/log/apt/term.log` across the entire run (1274 lines, 200+ packages installed, including the Lima dependency script's own apt-get) — every package was served from the cache seeded into `/var/cache/apt/archives` before the playbook ran. `apt cache seed`/`harvest` phases cost 12–28s each. | **Tier 2's win is real and not negligible on this host** — full elimination of package re-download on `--rebuild`, at a fixed ~30-50s round-trip seed+harvest cost. Whether this beats the plan's own anticipated "negligible once Tier 1 took the network off the critical path" finding depends on link speed; on this host (fast local mirror) the *time* savings are modest since downloads were already fast, but the *reliability/offline* benefit is unconditional. |
| Concurrency (two creates racing a stale/aging base; `--rebuild` under the lock; cancel-mid-reapply) | New in this plan (`baselock.go`) | **Not independently re-verified with two real concurrent `sand`/`limactl` processes** (time budget). Verified via the existing unit test suite under `go test -race ./internal/provision/... ./internal/lima/...` (all passed): `TestConcurrentCreatesBuildTheBaseOnce`, `TestConcurrentCreatesReapplyTheStaleBaseOnce`, `TestConcurrentCreatesRefreshTheAgedBaseOnce`, `TestRebuildDeletesTheBaseOnlyWhileHoldingTheBaseLock`, `TestCancelledReapplyReleasesTheBaseLock` — these use a real `flock`-based file lock and goroutines (not just a fake in-memory mutex), so they exercise the actual concurrency primitive, but against a fake `limactl` Runner rather than real VMs. | Unit-verified only; live multi-process verification remains an open gap. |
| Timings visible (TUI `l` key reopening the job log; progress bar unaffected) | Provisioning was a wall of unattributed output | Headless stdout: **confirmed live** — every run above printed `[timing]` phase lines and an end-of-run summary directly to stdout. TUI (`l` key reopening the retained job log; tile progress bar unaffected by the timing lines): **not independently re-verified live** (would require driving the Bubble Tea TUI); relied on code inspection (`internal/provision/timing.go`'s doc comment on why `step()` is never used for timing lines) and the passing unit test `TestPhaseTimer_NoStepCalls`. | Headless path live-verified; TUI path unit-test/code-inspection only. |
| CI parity | N/A | `go vet ./...`, `go test ./...`, `go test -race ./internal/provision/... ./internal/lima/...`, `gofmt -l .`, and `ansible-playbook --syntax-check site.yml` all green. `.github/workflows/test.yml`'s `lima-e2e` job configuration was inspected and confirmed to cover both an explicit `--rebuild` cold build and a warm reuse/re-apply create with the `_apt` keyring assertion intact — but see Findings: this job's live run was still marked "pending" as of this validation, and a live run **would have failed** on the regression below until this validator's fix landed. | Static checks green; live CI run status could not be confirmed from this environment (no GitHub Actions access) and the plan's own tracking says it was pending. |

#### Findings (in addition to the table above)

1. **CRITICAL — blocking regression, found and fixed by this validator.** `roles/user/tasks/main.yml`'s "Install fetched authorized_keys" task (introduced by task 02, commit `02c0ba1`, replacing the collection module `ansible.posix.authorized_key`) used `loop: "{{ lookup('ansible.builtin.file', '/tmp/' ~ user_name ~ '-github-keys').splitlines() }}"` guarded by a `when: user_github_keys_url | length > 0`. Ansible evaluates a task's `loop:` source expression before applying `when:`-based per-item skipping, so the `lookup()` call ran (and threw `Unable to access the file`) even though `user_github_keys_url` is empty by default and `sand` never sets it — meaning **every `sand create` with default settings crashed the base build** (`failed=1` in the play recap; `provisioning (base) failed for "sand-perf-base": exit status 2`). Reproduced in isolation with a minimal playbook before touching the real role file. Fixed with a minimal guard so the lookup itself is never evaluated when the URL is unset: `loop: "{{ (lookup('ansible.builtin.file', '/tmp/' ~ user_name ~ '-github-keys').splitlines()) if (user_github_keys_url | length > 0) else [] }}"`. Verified both branches (URL unset → skip cleanly; URL set → key installed) in isolation, then verified the real base build succeeds end-to-end with the fix. This was not caught by any existing check: `ansible-playbook --syntax-check` doesn't evaluate conditionals or lookups; the Go unit tests use a fake `limactl` Runner; the `limae2e`-tagged Go tests boot real VMs but never run the actual playbook; and the GitHub Actions `lima-e2e` job — the only thing that runs the real playbook end-to-end — had, per the plan's own Phase 5 tracking note, never actually been run live ("Task 11 ... completed (live CI run pending)"). **This finding means the plan's Task 11 (CI parity) and Task 13 gate should not be considered trustworthy until a live CI run is confirmed green with this fix.**
2. **P0 — Tier 1's headline bootstrap-slimming claim is not achieved as shipped, not fixed by this validator.** See the "Ansible bootstrap" table row above. `internal/provision/overlay.go`'s dependency-script `apt-get install` needs `--no-install-recommends` (or an equivalent `apt.conf.d` fragment) to actually stop pulling in the full `ansible` bundle via `ansible-core`'s `Recommends:`. Left unfixed because it is a design decision (whether suppressing Recommends could have unintended effects elsewhere in that same transaction) best made by the plan owner, and because — unlike finding 1 — it does not block measurement, only makes the measured numbers worse than the plan intended.
3. Both findings were discovered only by actually running the shipped code end-to-end on real hardware — neither was visible from code review, `go test ./...`, or `ansible-playbook --syntax-check` alone. This is the argument for why this task's Tier B (real VM execution) is not optional scope.

#### Limitations / unverified items

- No pre-plan ("before") baseline was measured on this host — the original bash-provisioner-era or pre-task-01 code was not checked out and timed separately; before-numbers above are the plan's own documented package-size/apt-pass-count facts, not independently re-measured wall-clock.
- Total wall-clock comparisons across separate runs carry noise from running up to 5–6 Lima VMs concurrently on one host during this validation session (host CPU/disk contention); the `base playbook`-phase comparisons (which isolate the actual apt/ansible work from VM boot) are more reliable than TOTAL comparisons for judging tier-by-tier attribution.
- Concurrency (two real racing `sand create` processes), the TUI job-log/progress-bar behavior, and the reboot-required bounce trigger were not independently re-verified live against real Lima VMs (time budget); all three are covered by passing unit/integration tests but not by this session's live-hardware pass.
- CI's actual (not configured) green status could not be confirmed from this environment.
